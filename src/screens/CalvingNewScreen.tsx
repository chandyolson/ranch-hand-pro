...
export default function CalvingNewScreen() {
  ...
  const handleReset = () => {
    ...
    setBirthWeight(''); setCalfSize('3'); setIsTwin(false); setTwin({ tag: '', sex: '', weight: '', size: '3' });
  ...
  return (
    <div style={{ maxWidth: 420, margin: '0 auto', backgroundColor: '#F5F5F0', minHeight: '100vh', overflow: 'hidden' }}>
      <div style={{ padding: '10px 16px 40px', display: 'flex', flexDirection: 'column', gap: 10 }}>
        ...
        {!contextOpen ? (
          ...
          <div style={{ borderRadius: 10, backgroundColor: 'white', border: '1px solid rgba(212,212,208,0.50)', padding: '12px' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
              <FieldRow label="Date"><input type="date" value={date} onChange={e => setDate(e.target.value)} style={IS} /></FieldRow>
              <FieldRow label="Group"><select value={group} onChange={e => setGroup(e.target.value)} style={{ ...IS, appearance: 'auto' as const }}><option>Spring Calvers</option><option>Fall Calvers</option><option>First Calf Heifers</option></select></FieldRow>
              <FieldRow label="Location"><select value={location} onChange={e => setLocation(e.target.value)} style={{ ...IS, appearance: 'auto' as const }}><option>Calving Barn</option><option>Home Place</option><option>East Pasture</option></select></FieldRow>
            </div>
        ...
        {/* ═══ DAM HISTORY PANEL ═══ */}
        {showDam && (
          <div style={{ borderRadius: 12, overflow: 'hidden', background: 'linear-gradient(145deg, #0E2646 0%, #163A5E 55%, #55BAAA 100%)' }}>
            {damTag ? (<>
              {/* ── Level 1: Summary ── */}
              ...
              <div style={{ backgroundColor: 'rgba(0,0,0,0.15)' }}>
                <button onClick={() => setDamFullHistory(!damFullHistory)} type="button" style={{
                ...
                {damFullHistory && (
                  <div style={{ padding: '0 16px 14px' }}>
                    ...
                    <div style={{ display: 'flex', gap: 0, borderRadius: 6, overflow: 'hidden', border: '1px solid rgba(255,255,255,0.10)', marginBottom: 10 }}>
                      {[{ k: 'calving', l: 'Calving' }, { k: 'work', l: 'Work' }, { k: 'treatments', l: 'Treatments' }].map(t => (
                        <button key={t.k} onClick={() => setDamHistoryTab(t.k)} type="button" style={{
                          ...
                          border: 'none', borderRight: t.k !== 'treatments' ? '1px solid rgba(255,255,255,0.08)' : 'none',
                        }}>{t.l}</button>
                      ))}
                    </div>
              ...
              <div style={{ textAlign: 'center', padding: '12px 16px' }}><span style={{ fontSize: 13, color: 'rgba(240,240,240,0.40)' }}>Type a dam tag to load history</span></div>
            )}
          </div>
        )}
        ...
        <div style={{ borderRadius: 12, backgroundColor: 'white', border: '1px solid rgba(212,212,208,0.60)', padding: '12px' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
            {/* Calf Tag */}
            <FieldRow label="Calf Tag"><input type="text" value={calfTag} onChange={e => setCalfTag(e.target.value)} placeholder="Tag number" style={IS} /></FieldRow>

            {/* Status */}
            <FieldRow label="Status">
              <SegmentedToggle value={calfStatus} onChange={setCalfStatus}
            ...
            <FieldRow label="Sex" req>
              <SegmentedToggle value={calfSex} onChange={setCalfSex}
            ...
            <FieldRow label="Tag Color">
              <div style={{ position: 'relative', flex: 1, minWidth: 0 }}>
                <span style={{ position: 'absolute', left: 12, top: '50%', transform: 'translateY(-50%)', width: 10, height: 10, borderRadius: 9999, backgroundColor: TAG_COLOR_HEX[calfColor] || '#999', zIndex: 1 }} />
                <select value={calfColor} onChange={e => setCalfColor(e.target.value)} style={{ ...IS, width: '100%', paddingLeft: 30, appearance: 'auto' as const }}>
        ...
        <Collapsible title="Details" badge={detailBits.length > 0 ? detailBits.length.toString() : null}
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 12, paddingTop: 4 }}>
            ...
            <div>
              <FieldRow label="Twin">
                <SegmentedToggle value={isTwin ? 'yes' : 'no'} onChange={v => setIsTwin(v === 'yes')} options={[{ value: 'no', label: 'No' }, { value: 'yes', label: 'Yes' }]} />
              </FieldRow>
              {isTwin && (
                <div style={{ backgroundColor: 'rgba(85,186,170,0.06)', borderRadius: 8, padding: '10px 12px', marginTop: 8, border: '1px solid rgba(85,186,170,0.15)' }}>
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                    <FieldRow label="Sex">
                      <SegmentedToggle value={twin.sex} onChange={v => setTwin(p => ({ ...p, sex: v }))}
                        options={[{ value: 'Bull', label: 'Bull' }, { value: 'Heifer', label: 'Heifer' }]}
                        colors={{ Bull: { bg: '#55BAAA', text: 'white' }, Heifer: { bg: '#E8A0BF', text: 'white' } }} />
                    </FieldRow>
                    <FieldRow label="Tag"><input type="text" value={twin.tag} onChange={e => setTwin(p => ({ ...p, tag: e.target.value }))} placeholder="Twin tag" style={IS} /></FieldRow>
                    <FieldRow label="Wt / Size">
                      <div style={{ display: 'flex', gap: 0, minWidth: 0, flex: 1 }}>
            ...
            <div>
              <FieldRow label="Graft">
                <SegmentedToggle value={isGraft ? 'yes' : 'no'} onChange={v => setIsGraft(v === 'yes')} options={[{ value: 'no', label: 'No' }, { value: 'yes', label: 'Yes' }]} />
              </FieldRow>
              {isGraft && (
                <div style={{ backgroundColor: 'rgba(243,209,42,0.06)', borderRadius: 8, padding: '10px 12px', marginTop: 8, border: '1px solid rgba(243,209,42,0.15)' }}>
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                    <FieldRow label="Foster Dam"><input type="text" value={graftDam} onChange={e => setGraftDam(e.target.value)} placeholder="Foster dam tag" style={IS} /></FieldRow>
                    <FieldRow label="Reason">
                      <select value={graftReason} onChange={e => setGraftReason(e.target.value)} style={{ ...IS, appearance: 'auto' as const, color: graftReason ? '#1A1A1A' : 'rgba(26,26,26,0.35)' }}>
