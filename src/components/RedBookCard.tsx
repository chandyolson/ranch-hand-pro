...
const tierColors: Record<string, { label: string; color: string; bg: string }> = {
  info:   { label: "INFO",   color: "#55BAAA", bg: "rgba(85,186,170,0.20)" },
  action: { label: "ACTION", color: "#F3D12A", bg: "rgba(243,209,42,0.20)" },
  urgent: { label: "URGENT", color: "#FF8A80", bg: "rgba(155,35,53,0.25)" },
};
...
const RedBookCard: React.FC<RedBookCardProps> = ({
  title, body, category, date, authorInitials, isPinned, hasAction, attachmentCount, flagTier, assignedTo, actionStatus, onClick,
}) => {
  const cat = categoryConfig[category];
  const tier = flagTier && tierColors[flagTier] ? tierColors[flagTier] : null;
  const isComplete = actionStatus === "complete" || actionStatus === "wont_do";

  return (
    <div
      className="bg-[#0E2646] rounded-xl px-4 py-4 cursor-pointer active:scale-[0.98] transition-all duration-150"
      style={{ opacity: isComplete ? 0.55 : 1, borderLeft: tier ? `3px solid ${tier.color}` : "none" }}
      onClick={onClick}
    >
      {/* Row 1 */}
      <div className="flex items-start justify-between gap-2">
        <div className="min-w-0 flex-1">
          <div className="flex items-center" style={{ fontSize: 15, fontWeight: 700, color: "white", lineHeight: 1.3, textDecoration: isComplete ? "line-through" : "none" }}>
            {isPinned && (
        ...
        <div className="shrink-0 flex items-center gap-1.5">
          {tier && (
            <span className="rounded-full" style={{ fontSize: 9, fontWeight: 700, letterSpacing: "0.06em", padding: "2px 8px", backgroundColor: tier.bg, color: tier.color }}>
              {tier.label}
            </span>
          )}
      ...
      <div className="flex items-center gap-2 mt-2.5">
        ...
        {assignedTo && (
          <span style={{ fontSize: 11, fontWeight: 500, color: "rgba(240,240,240,0.55)" }}>→ {assignedTo}</span>
        )}
        {isComplete && (
          <span className="rounded-full" style={{ fontSize: 9, fontWeight: 700, padding: "1px 6px", backgroundColor: "rgba(85,186,170,0.20)", color: "#55BAAA" }}>
            {actionStatus === "complete" ? "DONE" : "WON'T DO"}
          </span>
        )}
